...
const l = await fs.readFile('Authority.json', { encoding: 'utf8' });
...
for (let item of profiles) {
	console.log(no)
	no = item['Authority'] //.substring(0,4)
	no = no.replaceAll(' ','-')
	no = no.toLowerCase()
	const filename = dir + no+'-human-design.md'
	...
	let profileName = 'Human Design '+item['Authority']
	console.log(profileName)
	
	const templateA = `you are human design teacher.\n 
	we are speaking about "{product}".\n
	please write me engaging, attractive, informative (rich in details) short article (skip title as this is part of larger text) with sections: keynotes, why it's matter and challenges formated and structured in markdown
	`;
	...
	const templateD = `you are human design teacher. \n
	we are speaking about "{product}\n"
	make this text """\n
	{instr}\n
	"""
	more engaging, informative and attractive paragraph (without title, introduction or conclusion - this part of larger text) structured and formated (bold text, subtitles) as markdown
	`
		...
		const chainD = new LLMChain({ llm: model, prompt: promptD });
		...
		const resD = await chainD.call({ product: profileName,
			instr: item['Suggestions / At Work']
		});
