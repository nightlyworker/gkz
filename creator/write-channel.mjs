...
const l = await fs.readFile('Channels.json', { encoding: 'utf8' });
...
for (let item of profiles) {
	console.log(no)
	no = item['Name'].substring(0,4)
	const filename = dir + 'channel-'+no+'-human-design.md'
	...
	//process.exit(1)	
	let profileName = 'Human Design '+item['Name']
	console.log(profileName)
	
	const templateA = `you are human design teacher.\n 
	we are speaking about "{product}".\n
	please write me engaging, attractive, informative (rich in details) short article (skip title as this is part of larger text) with sections: keynotes, why it's matter and gifts formated and structured in markdown
	`;
